...
class TestTrainResnet(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.model_dir = 'climex/tests/testmodels_results/ResNet/'
    ...
    def test_argument_checks(self):
        ...
        # Test for kernel_size
        with self.assertRaises(SystemExit) as cm:
            train_resnet(kernel_size=-1)
        self.assertEqual(cm.exception.args[0], '`kernel_size` must be greater than 0.')
        with self.assertRaises(SystemExit) as cm:
            train_resnet(kernel_size="1")
        self.assertEqual(cm.exception.args[0], '`kernel_size` must be an integer.')

        # Tests for stride
        with self.assertRaises(SystemExit) as cm:
            train_resnet(stride=-1)
        self.assertEqual(cm.exception.args[0], '`stride` must be greater than 0.')
        with self.assertRaises(SystemExit) as cm:
            train_resnet(stride="1")
        self.assertEqual(cm.exception.args[0], '`stride` must be an integer.')

        # Tests for padding
        with self.assertRaises(SystemExit) as cm:
            train_resnet(padding=-1)
        self.assertEqual(cm.exception.args[0], '`padding` must be greater than 0.')

        with self.assertRaises(SystemExit) as cm:
            train_resnet(padding="1")
        self.assertEqual(cm.exception.args[0], '`padding` must be an integer.')

        # Tests for use_weight
        with self.assertRaises(SystemExit) as cm:
            train_resnet(use_weight='not_a_bool')
        self.assertEqual(cm.exception.args[0], "`use_weight` must be a bool.")

        with self.assertRaises(SystemExit) as cm:
            train_resnet(use_weight=1)
        self.assertEqual(cm.exception.args[0], "`use_weight` must be a bool.")

        # Tests for weights
        with self.assertRaises(SystemExit) as cm:
            train_resnet(weights=1)
        self.assertEqual(cm.exception.args[0], "`weights` must be a float list of len 3 or None.")

        with self.assertRaises(SystemExit) as cm:
            train_resnet(weights=[1, 1, '1'])
        self.assertEqual(cm.exception.args[0], "`weights` must be a float list of len 3 or None.")

        # Tests for n_years_val and n_years_test
        with self.assertRaises(SystemExit) as cm:
            train_resnet(n_years_val=-1)
    ...
    def test_model_cust_weigths(self):
        ...
        try:
            ...
            train_resnet(batch_size=64, epochs=1, patience=7, use_weight=True, weights=[0.05, 0.45, 0.5],
