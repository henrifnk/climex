...
def train_conv_lstm(batch_size=256, epochs=100, patience=7, lr=0.01, use_weight=True, weights=None,
                    n_years_val=10, n_years_test=10, splitting_method='sequential', time_depth=8,
                    season=None, path_to_data='climex/tests/testdata/training_database_daily_unit_tests.nc',
    ...
    check_args_train_conv_lstm(batch_size, epochs, patience, lr, use_weight, weights,
    ...
    # Set up logging file
    logger = LogModel(model_dir=model_dir, model_name="ConvLSTM")
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for i, data in enumerate(train_loader):
            ...
            # forward + backward
            output = rnn_decoder(cnn_encoder(inputs))  # output has dim = (batch, time_step, num_classes)
...
def check_args_train_conv_lstm(batch_size, epochs, patience, lr, use_weight, weights,
                               n_years_val, n_years_test, splitting_method, season, path_to_data, model_dir):
    """Argument checks for `train_conv_lstm` function.
    """
